...
Python-applikationen använder **SQLAlchemy Core** för att hantera databasanslutningen. Funktionerna i `queries.py` använder **parameteriserade queries** (med `:brand` och `:email` placeholders) för att förhindra SQL Injection-attacker, vilket är ett viktigt säkerhetskrav.
